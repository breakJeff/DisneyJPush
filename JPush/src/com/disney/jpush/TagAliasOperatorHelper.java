...
/**
 * Deal with tagalias related logic
 * */
public class TagAliasOperatorHelper {
    ...
    private boolean RetryActionIfNeeded(int errorCode,TagAliasBean tagAliasBean){
        ...
        //error code is 6002 timeout,6014 server is busy,retry after a while
        if(errorCode == 6002 || errorCode == 6014){
            ...
            if(tagAliasBean!=null){
                ...
                PushUtil.showToast(logs, context);
                return true;
            }
        }
        return false;
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        init(context);
        // Get cache result by previous operation by sequence
        TagAliasBean tagAliasBean = tagAliasActionCache.get(sequence);
        if(tagAliasBean == null){
            PushUtil.showToast("获取缓存记录失败", context);
            return;
        }
        if(jPushMessage.getErrorCode() == 0){
            ...
            if(jPushMessage.getErrorCode() == 6018){
                //tag number exceeds ,clear part of it than add it
